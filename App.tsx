  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-slate-900 font-sans select-none">
      ...
      <div className="absolute bottom-8 left-8 pointer-events-none opacity-60 z-10 text-white">
        <h1 className="text-5xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-cyan-200 drop-shadow-lg flex items-center gap-3">
          TouchFish
      ...
      {/* Instructions Tip */}
      <div 
        className={`absolute bottom-10 left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-xl px-8 py-4 rounded-full text-white font-medium border border-white/20 shadow-[0_0_20px_rgba(34,211,238,0.1)] transition-opacity duration-1000 pointer-events-none flex items-center gap-3 ${showTip ? 'opacity-100' : 'opacity-0'}`}
      >
